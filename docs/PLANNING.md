...
- ステータス管理: 読みたい / 読書中 / 読了
- セッションに **進捗率(%)** を記録(スライダー入力)。100% で読了候補として提示
- 読了時に一言感想・5 段階評価
...
├ progressPercent: Int?   // reading のとき(セッション終了時点の進捗 0–100)
...
- 読書の進捗は % で持つ(ページ数は持たない)。セッションごとに「終了時点の進捗 %」を記録し、Book 側に最新値を反映。差分(そのセッションで進んだ %)は前セッションとの差から導出。
